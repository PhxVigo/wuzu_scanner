...
class GeeNFCReaderPro:
    ...
    def read_reserved(self, pwd=b"\x00\x00\x00\x00"):
        return self.read_data_for_single_tag(
            mem=0x00,
            word_ptr=0x00,
            word_count=4,
            pwd=pwd,
    ...
    def write_access_password(self, new_pwd: bytes, current_pwd=b"\x00\x00\x00\x00"):
        """
        Write the Access Password to Reserved memory using EPC-targeted Write Data.
        REQUIRED for Monza R6-P + UR-2000.
        """
        if len(new_pwd) != 4:
        ...
        self.write_data_for_single_tag(
            mem=0x00,
            word_ptr=0x02,
            words=[w1, w2],
            pwd=current_pwd
    ...
    def read_tid(self):
        data = self.read_data_for_single_tag(
            mem=0x02,
            word_ptr=0x00,
            word_count=6,
        )
    ...
    # -------------------------------------------------------------------------
    # Read memory using EPC targeting (safer for TID/Reserved)
    # -------------------------------------------------------------------------

    def read_data_for_single_tag(self, mem, word_ptr, word_count, pwd=b"\x00\x00\x00\x00"):
        """
        Read memory from the one tag in the field using proper EPC-targeted
        Read Data formatting for the UR-2000 and Monza R6-P.

        This fixes 0xFC errors by ensuring ENum is always EPC-length-in-words.
        """

        # 1) Get tag EPC
        epc_hex = self.require_single_tag()
        epc_bytes = bytes.fromhex(epc_hex)

        # EPC length *in words* (96-bit EPC = 6 words)
        epc_word_len = len(epc_bytes) // 2

        # 2) Build Data[] correctly:
        #    [ENum] [EPC bytes] [Mem] [WordPtr] [NumWords] [Pwd] [MaskAdr] [MaskLen]
        data = bytes([epc_word_len]) + epc_bytes + bytes([
            mem,
            word_ptr,
            word_count,
        ]) + pwd + bytes([0x00, 0x00])
        ...
        if reCmd != 0x02:
            raise RuntimeError(f"Unexpected read_data_for_single_tag response reCmd=0x{reCmd:02X}")

        if status != 0x00:
            raise RuntimeError(f"read_data_for_single_tag failed, status=0x{status:02X}")

        return rdata

    # -------------------------------------------------------------------------
    # Write memory using EPC targeting (required for Reserved writes)
    # -------------------------------------------------------------------------

    def write_data_for_single_tag(self, mem, word_ptr, words, pwd=b"\x00\x00\x00\x00"):
        """
        Write memory to the one tag in the field using EPC-targeted Write Data.
        This is REQUIRED for writing Reserved memory on Monza R6-P.
        """
        # 1) Get EPC
        epc_hex = self.require_single_tag()
        epc_bytes = bytes.fromhex(epc_hex)
        ...
        data = bytes([
            len(words),          # WNum
            epc_word_len,        # ENum (EPC word count)
        ]) + epc_bytes + bytes([
            mem,
            word_ptr,
        ]) + wbytes + pwd + bytes([0x00, 0x00])
        ...
        if reCmd != 0x03:
            raise RuntimeError(f"Unexpected write_data_for_single_tag reCmd=0x{reCmd:02X}")

        if status != 0x00:
            raise RuntimeError(f"write_data_for_single_tag failed, status=0x{status:02X}")
